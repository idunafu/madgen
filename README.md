...
| `--lyrics-selection core/legacy` | core | 実際に使うWORLDコアのF0・伸長量で選択 / 従来のDB区間全体で選択 |
| `--lyrics-consonants aligned/legacy` | aligned | 子音の立ち上がり・母音前配置・増幅上限を調整 / 従来の子音処理 |
...
2. **ピッチ＋長さコスト**: 既定の `core` では母音・「ん」の実使用WORLDコアのF0と目標ピッチとの差（しきい値以内は0、それを超えると半音ごとに+1）、伸長負担（2倍につき3段階、最大10）の和で評価します。音素コストが最良で同点の候補を解析し、選択した解析・使用フレームを合成でも使います。子音は従来の長さ不足率を使い、ピッチを見ません。`--lyrics-selection legacy` または `--no-lyrics-stretch` の場合は、従来の区間全体のF0・長さで評価します。
...
子音はピッチ補正・伸縮をせず、既定の `aligned` では短い素材を隣接する母音の直前へ配置します。無声破裂音に明確な立ち上がりがある場合だけ、その2ms前から切り出します。他の子音は注釈の開始位置を保ちます。音量は無音を除く活動区間から計算し、母音より6dB低い目標へ調整しますが、増幅は最大6dBに制限します。母音の目標音量との差を常に6dBにする処理ではありません。`--lyrics-consonants legacy` で従来の配置・音量処理に戻せます。

聴取確認を経て、コア選択と子音調整を既定にしました。従来処理へ戻すには `--lyrics-selection legacy --lyrics-consonants legacy` を指定します。新しい解析キャッシュはDB横の `.cache/world-core-v1/` に保存し、音素DBの再構築や追加依存は不要です。初回の候補解析には時間がかかります。`plan.json` の `world_core` / `world_consonant` に実際の使用位置と加工内容を記録します。[実装・比較記録](docs/world-improvements.md)を参照してください。
...
進捗は文字起こし・音素解析それぞれの完了素材数で表示します。WhisperXの `file %` は現在の素材内の進捗です。音声検出と最初の推論結果を待つ間は割合を表示せず、素材番号・ファイル名・処理内容を表示します。ログには短い素材の完了も記録し、音素解析でCPU後処理やDB保存を待つ状態も区別します。
